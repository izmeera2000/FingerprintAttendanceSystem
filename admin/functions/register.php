...
if (isset($_POST['user_register'])) {
  ...
  if (empty($_POST['waris_nama'])) {
    ...
    $waris_nama = mysqli_real_escape_string($db, $_POST['waris_nama']);
  ...
  if (empty($_POST['waris_tel'])) {
    ...
    $waris_tel = mysqli_real_escape_string($db, $_POST['waris_tel']);
  ...
  if (empty($_POST['bengkel'])) {
    $errors['bengkel'] = "Bengkel required";
  } else {
...
if (isset($_POST['user_register2'])) {
  ...
  if (isset($ndp) && isset($email) && isset($phone) && isset($kp)) {
    ...
    $user_check_query = "SELECT * FROM user WHERE   email='$email'  OR phone='$phone'    OR kp='$kp'   LIMIT 1";
    $result = mysqli_query($db, $user_check_query);
    $user = mysqli_fetch_assoc($result);
  ...
  if (count($errors) == 0) {
    ...
    $query = "INSERT INTO user (role,password, nama,email,phone,kp,jantina,agama,status_kahwin,bangsa,bengkel_id) 
                          VALUES('$role','$password','$fullname','$email','$phone','$kp','$jantina','$agama','$statuskahwin','$bangsa','$bengkel')";
    mysqli_query($db, $query);


    //verify
    $query = "SELECT a.*, b.nama as role_name FROM `user` a INNER JOIN user_role b ON a.role = b.id WHERE ( email='$email') AND password='$password'";
    $results = mysqli_query($db, $query);
    $user3 = mysqli_fetch_assoc($results);
    $id3 = $user3['id'];
    $filename = uploadpic_id($user3['id'], $errors);
